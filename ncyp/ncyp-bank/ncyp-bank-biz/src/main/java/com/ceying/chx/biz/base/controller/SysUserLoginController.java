...
public class SysUserLoginController {
    ...
    public RespJson login(HttpServletRequest request,@Validated(LoginValid.class) @RequestBody SysUserDto sysUserDto, Errors errors){
        if (errors.hasErrors()) {
        ...
        try {
            ...
            System.out.println("ncyp-bank-biz 登录controller  sessionId : "+session.getId());
            //把登录信息传回front，front中存入session
            Map<String,Object> map=new HashMap();
            map.put(UserConstant.USER_LOGIN_SESSION, loginUser);
            map.put(UserConstant.USER_SESSION,user);
            respJson.setData(map);
            respJson.setTotalCount(null);
        } catch (Exception e) {
            e.printStackTrace();
            respJson = RespJsonFactory.buildFailure("登录异常"+e.getMessage());
        }
        return respJson;
    }
}
