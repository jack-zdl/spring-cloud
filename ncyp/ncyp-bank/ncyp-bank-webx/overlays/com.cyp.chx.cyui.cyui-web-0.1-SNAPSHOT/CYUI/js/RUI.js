...
;(function ($, undefined) {
    $.widget('RUI.RUploadfile', {
        ...
        setTable: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                //新增了下载功能
                $('#' + ID + ' table').append('<tr><td><span index="'+data[i].storageid+'" class=" glyphicon glyphicon-remove-circle remove_file"></span>&nbsp;<a href="'+ME.options.downloadUrl+'?ids='+data[i].storageid+'" class=" glyphicon glyphicon-download download_file"></a></td><td>' + data[i].filename + '</td><td>' + data[i].filesize + '</td><td>' + data[i].creatorname + '</td><td>' + data[i].createdate + '</td></tr>');
            }
