...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			if($t.p.tblwidth) {
				if((nwidth - $t.p.tblwidth) > 10){
                   $t.p.tblwidth = $t.p.tblwidth + 17;
				}
				// if(){
				...
				if($($t.grid.bDiv).children('div').width() < $t.p.tblwidth){
                    $('table:first').css("width",($t.p.tblwidth - 18)+"px");
				};
				// console.log($($t.grid.bDiv).children('div'));
				// console.log($($t.grid.bDiv));
// 				console.log($($t.grid.bDiv).width())
// $($t.grid.bDiv).children('div').css('border','1px solid red');
                // if($($t.grid.bDiv).children('div').children('table').outerWidth() > ($($t.grid.bDiv).width()){
                // 	alert(1);
                // }
				// console.log($($t.grid.bDiv).children('div').children('table').outerWidth());
			}
		});
	},
	...
	gridResize : function(opts) {
		return this.each(function(){
			...
			opts.stop = function (ev, ui) {
				...
				$($t).jqGrid('k',ui.size.width,opts.shrinkToFit);
