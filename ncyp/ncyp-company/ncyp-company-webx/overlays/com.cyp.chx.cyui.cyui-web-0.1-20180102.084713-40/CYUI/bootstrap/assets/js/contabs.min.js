$(function() {
    ...
    function g(n) {
        ...
        var p = 0;
        if ($(".page-tabs-content").outerWidth(false) < k) {
            p = 0;
        } else {
            if (q <= (k - $(n).outerWidth(true) - $(n).next().outerWidth(true)- $(n).prev().outerWidth(true))) {

                if ((k - $(n).next().outerWidth(true)) > q) {
                    p = o;
                    var m = n;
                    while ((p - $(m).outerWidth(true)) > ($(".page-tabs-content").outerWidth(false) - k)) {
                        p -= $(m).prev().outerWidth(true);
                        m = $(m).prev();
                    }
                }
            } else {
    ...
    function ding(n){
        ...
        if ($(".page-tabs-content").outerWidth(false) < k) {
            ...
            p = o - $(n).prev().outerWidth(true)-$(n).outerWidth(true)+$(n).next().outerWidth(true);
        }
        $(".page-tabs-content").animate({
            marginLeft: 0 - p + "px"
        }, "fast");
    ...
    function c() {
        ...
        $(this).parent('li').parents('li').siblings().find('li').removeClass('active open').end().removeClass("active open").end().addClass("active open");
    ...
    function j() {
        ding($(".J_menuTab.active"));
    }
    $(".J_tabShowActive").on("click", j);
    function e() {
        var $J_menuTab = $(this);
        $('.nav-list').find('li').each(function(index, el) {
            if ($(this).children('a').attr('href')===$J_menuTab.attr('data-id')) {
                $(this).siblings().removeClass('active').end().addClass('active');
                $(this).parents('li').siblings().find('li').removeClass('active open').end().removeClass("active open").end().addClass("active open");
                return false;
            }
        });
        if (!$(this).hasClass("active")) {
