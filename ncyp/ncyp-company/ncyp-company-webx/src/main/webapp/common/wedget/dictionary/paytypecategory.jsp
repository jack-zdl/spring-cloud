...
/***
 * 交易类型类别：0：未分类，1：上划，2：下拨，3：提现，4：存现
 * */
...
<r:combo check="<%=check%>" id="<%=id%>" name="<%=id%>" title="${param.title}" autoload="true" width="${param.width}"
         ...
         displayFormat="{displayField}" beforeSelect="<%=BFn %>" afterSelect="${param.SelFn}"
         dataUrl="${pageContext.request.contextPath}/common/queryDicOption.do?keyno=1072"/>